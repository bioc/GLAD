...
Two functions are available: \emph{glad} and \emph{daglad}. The second one is an improvment of of first one which was originally describes in \cite{hupe04}. We recommand to use the \emph{daglad} function.
...
plotProfile(res, unit=3, Bkp=TRUE, labels=FALSE, plotband=FALSE, Smoothing="Smoothing")
@
\end{center}
...
\begin{center}
<<fig=TRUE,echo=FALSE,width=17,height=12>>=
plotProfile(profileCGH, Smoothing="Smoothing", Bkp=TRUE, plotband=FALSE)
