...
arrayPlot.default <-function(Statistic, Col, Row,
			      ...
			      zlim=zlim, bar=TRUE, ...)
{
	...
	image(1:ncol, 1:nrow, z, axes=FALSE, col=col, xlab="", ylab="", ...)

	if(contour)
		contour(1:ncol,1:nrow, z, nlevels=nlevels, add=TRUE)

	box(lwd=2)
	abline(v=((1:ArrCol-1)*SpotCol + 0.5),lwd=1)
	abline(h=((1:ArrRow-1)*SpotRow + 0.5),lwd=1)
...
arrayPlot.arrayCGH <-function(arrayCGH, variable,
			      mediancenter=FALSE,
			      col=myPalette("green","red","yellow"),
			      contour=FALSE, nlevels=5,
			      zlim=zlim, bar=TRUE, ...)
{
   ...
   if(length(which(names(arrayCGH$arrayValues)==variable))<1)stop(paste("variable",variable,"not found",sep=" "))
	...
	box(lwd=2)
	abline(v=((1:arrayCGH$arrayDesign[1]-1)*arrayCGH$arrayDesign[3] + 0.5),lwd=1)
	abline(h=((1:arrayCGH$arrayDesign[2]-1)*arrayCGH$arrayDesign[4] + 0.5),lwd=1)
