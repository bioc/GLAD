...
tkdaglad <- function(list=character(0), ...)
  {
    ...
    genomestep.frm <- tkframe(frameOverall, borderwidth=2, relief="groove")
    tkpack(tklabel(genomestep.frm,text="Genome Step"), anchor="w")

    normalrefcenter <- tclVar(0)
    cb.normalrefcenter <- tkcheckbutton(genomestep.frm, variable=normalrefcenter, text="normalrefcenter")

    genomestep <- tclVar(0)
    cb.genomestep <- tkcheckbutton(genomestep.frm, variable=genomestep, text="genomestep")
    tkpack(cb.genomestep,cb.normalrefcenter, side="left")

    tkpack(genomestep.frm, fill="x")
    ...
    amplicon <- tclVar(1)
    l.amplicon <- tklabel(region.frm.left, text="amplicon:")
    e.amplicon <- tkentry(region.frm.right, width=10, textvariable=amplicon)
    tkpack(l.amplicon, anchor="e")
    tkpack(e.amplicon,anchor="w")

    deletion <- tclVar(-5)
    l.deletion <- tklabel(region.frm.left, text="deletion:")
    e.deletion <- tkentry(region.frm.right, width=10, textvariable=deletion)
    tkpack(l.deletion, anchor="e")
    tkpack(e.deletion, anchor="w")

    deltaN <- tclVar(0.1)
    l.deltaN <- tklabel(region.frm.left, text="deltaN:")
    e.deltaN <- tkentry(region.frm.right, width=10, textvariable=deltaN)
    tkpack(l.deltaN, anchor="e")
    tkpack(e.deltaN, anchor="w")


    forceGL1 <- tclVar(-0.15)
    l.forceGL1 <- tklabel(region.frm.left, text="forceGL[1]:")
    e.forceGL1 <- tkentry(region.frm.right, width=10, textvariable=forceGL1)
    tkpack(l.forceGL1, anchor="e")
    tkpack(e.forceGL1, anchor="w")
  
    forceGL2 <- tclVar(0.15)
    l.forceGL2 <- tklabel(region.frm.left, text="forceGL[2]:")
    e.forceGL2 <- tkentry(region.frm.right, width=10, textvariable=forceGL2)
    tkpack(l.forceGL2, anchor="e")
    tkpack(e.forceGL2, anchor="w")
    
    tkpack(region.frm.left,region.frm.right, side="left")
    tkpack(regionall.frm, fill="x")
    tkpack(region.frm)

    Bkp.frm <- tkframe(frameOverall, borderwidth=2, relief="groove")
    tkpack(tklabel(Bkp.frm, text="Remove Breakpoints with small weight"), anchor="w")

    Bkp.frm.left <- tkframe(Bkp.frm)
    Bkp.frm.right <- tkframe(Bkp.frm)

    nbsigma <- tclVar(3)
    l.nbsigma <- tklabel(Bkp.frm.left, text="nbsigma:")
    e.nbsigma <- tkentry(Bkp.frm.right, width=10, textvariable=nbsigma)
    tkpack(l.nbsigma,anchor="e")
    tkpack(e.nbsigma, anchor="w")
 
    MinBkpWeight <- tclVar(0.35)
    l.MinBkpWeight <- tklabel(Bkp.frm.left, text="MinBkpWeight:")
    e.MinBkpWeight <- tkentry(Bkp.frm.right, width=10, textvariable=MinBkpWeight)
    tkpack(l.MinBkpWeight, anchor="e")
    tkpack(e.MinBkpWeight, anchor="w")

    tkpack(Bkp.frm.left, Bkp.frm.right, side="left")
    tkpack(Bkp.frm, fill="x")


    pos.frm <- tkframe(frameOverall, borderwidth=2, relief="groove")
    tkpack(tklabel(pos.frm, text="Check accuracy of breakpoint position"), anchor="w")

    
  
    CheckBkpPos <- tclVar(1)
    cb.CheckBkpPos <- tkcheckbutton(pos.frm, variable=CheckBkpPos, text="CheckBkpPos")
    tkpack(cb.CheckBkpPos)

    tkpack(pos.frm, fill="x")
    ...
    OnAnalysis <- function()
      {
        ...
        normalrefcenterVal <- FALSE
        if (tclvalue(normalrefcenter)!="0")
          normalrefcenterVal <- TRUE

        genomestepVal <- FALSE
        if (tclvalue(genomestep)!="0")
          genomestepVal <- TRUE
        ...
        ampliconVal <- as.numeric(tclvalue(amplicon))
        deletionVal <- as.numeric(tclvalue(deletion))
        deltaNVal <- as.numeric(tclvalue(deltaN))
        forceGL1Val <- as.numeric(tclvalue(forceGL1))
        forceGL2Val <- as.numeric(tclvalue(forceGL2))
        nbsigmaVal <- as.numeric(tclvalue(nbsigma))
        MinBkpWeightVal <- as.numeric(tclvalue(MinBkpWeight))

        CheckBkpPosVal <- FALSE
        if (tclvalue(CheckBkpPos)!="0")
          CheckBkpPosVal <- TRUE
        ...
        for (array in list)
          {
            ...
            tmp <- daglad(tmp,
                          ...
                          normalrefcenter=normalrefcenterVal,
                          genomestep=genomestepVal,
                          ...
                          alpha=alphaVal,
                          msize=msizeVal,
                          amplicon=ampliconVal,
                          deletion=deletionVal,
                          deltaN=deltaNVal,
                          forceGL=c(forceGL1Val,forceGL2Val),
                          nbsigma=nbsigmaVal,
                          MinBkpWeight=MinBkpWeightVal,
                          CheckBkpPos=CheckBkpPosVal)
        ...
        tkmessageBox(title="DAGLAD Analysis",message="The analysis is finished",icon="info",type="ok")
    ...
    OnDefault <- function()
      {
        ...
        tclvalue(normalrefcenter) <- 0
        tclvalue(genomestep) <- 0
        ...
        tclvalue(deletion) <- -5
        tclvalue(deltaN) <- 0.1
        tclvalue(forceGL1) <- -0.15
        tclvalue(forceGL2) <- 0.15
        tclvalue(nbsigma) <- 3
        tclvalue(MinBkpWeight) <- 0.35
        tclvalue(CheckBkpPos) <- 1
      }
